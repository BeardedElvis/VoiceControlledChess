...
class _menu(_menu_object):
    ...
    def update_objects(self,mx,my,bx,by,lc,e,char):
        """Updates enabled buttons by making them selected, pressed or changing their entry values."""
        self.pressed_buttons = []

        for obj in self.objects:

            if self.id == "game" and not obj.id == "new" and not obj.id == "quit" and not obj.id == "column" and not obj.id == "row":
                x = bx
                y = by
                c = e
            else:
...
def update_menus(mx,my,bx,by,lc,e,char):
    ...
    menu_list[order_selected[-1]].update_objects(mx,my,bx,by,lc,e,char)
