...
train_eval_data = pd.read_csv(root_dir + '/data/train.tsv', sep = '\t')

if not os.path.isfile(root_dir + '/train.csv'):
    ...
    train_data.to_csv(root_dir + '/train.csv')
    eval_data.to_csv(root_dir + '/eval.csv')
else:
    train_data = pd.read_csv(root_dir + '/train.csv')
    eval_data = pd.read_csv(root_dir + '/eval.csv')
...
#     libri_frame = pd.DataFrame([ (a, b) for a, b in libri_texts() ], columns=['path', 'sentence'])
...
#     vox_frame = pd.DataFrame([ (a, b) for a, b in vox_texts() ], columns=['path', 'sentence'])
#     vox_frame.to_csv('vox.csv')
# else:
#     vox_frame = pd.read_csv('vox.csv')

test_data = pd.read_csv(root_dir + '/data/test.tsv', sep = '\t')

train_data['path'] = train_data['path'].apply(lambda f: 'training/{}'.format(f))

eval_data['path'] = eval_data['path'].apply(lambda f: 'training/{}'.format(f))
test_data['path'] = test_data['path'].apply(lambda f: 'testing/{}'.format(f))

# if not os.path.isfile('full_train.csv'):
#     train_data = train_data[['path', 'sentence']].append(libri_frame).append(vox_frame)
...
def print_alphabet():
    ...
    for ix, data_frame in enumerate(data_frames):
        ...
        data_frame.apply(lambda row: [ c for c in row.sentence ], axis=1).apply(uniques.update)
        print('Done!')
...
def input_fn(input_dataset, params, load_wave_fn=load_wave):
    def _input_fn():
        ...
        if 'max_text_length' in params and params['max_text_length'] is not None:
            ...
            dataset = input_dataset[input_dataset.sentence.str.len() < params['max_text_length']]
            
        if 'min_text_length' in params and params['min_text_length'] is not None:
            print('Constraining dataset to the min_text_length')
            dataset = input_dataset[input_dataset.sentence.str.len() >= params['min_text_length']]
...
def model_fn(features, labels, mode, params):
    ...
    sentence, predicted_codes = decode_logits(logits, lengths, params)

    if mode == tf.estimator.ModeKeys.PREDICT:
        predictions = {
            'logits': logits,
            'sentence': tf.sparse_tensor_to_dense(
                sentence,
                ''
            )
        }
        ...
        if mode == tf.estimator.ModeKeys.EVAL:            
            ...
            tf.summary.sentence(
                ...
                tf.sparse_tensor_to_dense(sentence, '')
            )
...
class TestNotebook(unittest.TestCase):
    ...
    @settings(max_examples=10, deadline=None)
    def test_encode_and_decode_work(self, sentence):
        assume(sentence != '')
        
        params = { 'alphabet': '!"&\',-.01234:;\abcdefghijklmnopqrstuvwxyz' }
        
        label_ph = tf.placeholder(tf.string, shape=(1), name='sentence')
        ...
        with tf.Session() as session:        
            ...
            self.assertEqual(sentence, ''.join(map(lambda s: s.decode('UTF-8'), decoded.values)))
            self.assertEqual(codes.values.dtype, np.int32)
            self.assertEqual(len(codes.values), len(sentence))
    ...
    def test_dataset_returns_data_in_order(self):
        ...
        data = pd.DataFrame(
            data={
                'sentence': [ str(i) for i in range(10)],
                'path':  [ '{}/wav'.format(i) for i in range(10)]
            }
        )
...
[ b''.join(t['sentence']) for t in test_results ] """